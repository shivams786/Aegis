...
Policy simulation replay is leased through PostgreSQL rather than performed inline with bundle publication. Simulation workers claim bounded leases, publish redacted simulation-work events, validate referenced bundle hashes, choose a bounded recent-invocation sample by scope, compare baseline and proposed decisions from bundle metadata, and record terminal findings back to the durable run row. Running archived OPA bundle artifacts during replay remains a later policy-engine adapter behind the same run row contract.
