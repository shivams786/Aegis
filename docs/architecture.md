...
Policy bundles are registered durably before rollout. Each tenant can have only one active bundle, activation retires the previous active bundle, and registration/activation both emit redacted outbox events. Replay runs store baseline and proposed version/hash pairs so replay work can be leased asynchronously without exposing sampled invocation bodies in the control-plane event stream. The current replay worker compares policy decisions from bundle metadata and persists only redacted decision deltas.

Background workers handle tasks that should not be coupled to request latency: outbox delivery, approval expiry, stale reservation cleanup, reconciliation of unknown downstream outcomes, audit-root generation, and policy replay. The current worker drains PostgreSQL outbox rows to NATS, expires stale approvals, releases stale budget reservations, leases persisted unknown-outcome invocations for reconciliation, replays policy decisions from bounded samples, persists redacted findings, and generates audit roots.
...
The MCP gateway exposes Streamable HTTP. `tools/list` filters tools the caller could potentially use, but `tools/call` still re-enters the full invocation pipeline. Aegis never accepts a token issued for a downstream MCP server as authority to access Aegis itself.
